    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity >= 1:
                ...
                order_item.save()
                messages.info(request, f"{item.laptop_title} quantity has been updated")
                return redirect("App_Order:cart")
        else:
            messages.info(request, f"{item.laptop_title} is not in your cart")
            return redirect("home")
    else:
        messages.info(request, "You don't have an active order")
        return redirect("home")
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity > 1:
                ...
                order.orderitems.remove(order_item)
                order_item.delete()
                messages.warning(request, f"{item.laptop_title} item has been removed from your cart")
                return redirect("App_Order:cart")
        else:
            messages.info(request, f"{item.laptop_title} is not in your cart")
            return redirect("home")
